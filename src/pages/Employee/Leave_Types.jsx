...
const Leaves = () => {
  const { getPaginatedLeaves, createLeave, updateLeave, removeLeave, loading } = useLeave();
  ...
  }, [getPaginatedLeaves, currentPage, itemsPerPage, searchTerm]);
  ...
  const handleOpenModal = (leave = null) => {
    if (leave) {
      ...
      setFormData({ 
        ...
        paidLeave: leave.paidLeave || "Inactive",
              ...
              {loading && leaves.length === 0 ? (
                ...
                leaves.map((l) => (
                  <tr key={l._id} className="hover:bg-primary/5 transition-colors border-b">
                    ...
                    <td>
                      <div className={`badge ${l.status === 'Active' ? 'badge-success' : 'badge-error'} badge-outline font-bold text-[9px] uppercase`}>
                        {l.status}
                      </div>
                    </td>
