    ...
    <div className="p-6 bg-base-200 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-2xl shadow-sm mb-6 border-l-8 border-primary">
        <div>
          ...
          <p className="text-neutral-500 font-medium font-sans">Manage Payment Methods & Payees</p>
        </div>
      ...
      <div className="bg-base-100 rounded-2xl shadow-sm border border-base-300">
          ...
          <table className="table w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="4" className="text-center py-10 opacity-50 font-bold">No payment types found.</td></tr>
              ) : (
                paymentTypes.map((pt) => (
                  <tr key={pt._id} className="hover:bg-primary/5 transition-colors border-b border-base-200">
                    <td>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-bold text-secondary text-base">{pt.name}</div>
                      </div>
                    </td>
                    <td>
                      <div className="badge badge-ghost font-mono text-xs font-bold">{pt.refName}</div>
                    </td>
                    <td>
                    ...
                    <td className="text-center">
                      <div className="flex justify-center gap-1">
                        <button onClick={() => handleOpenModal(pt)} className="btn btn-sm btn-circle btn-ghost text-info hover:bg-info/10">
                          <FaEdit className="text-lg" />
                        </button>
                        <button onClick={() => handleDelete(pt._id)} className="btn btn-sm btn-circle btn-ghost text-error hover:bg-error/10">
        ...
        <div className="p-4 flex flex-col md:flex-row justify-between items-center gap-4 bg-base-50 rounded-b-2xl border-t">
          <div className="text-sm font-medium text-neutral-500">
            Showing <span className="text-secondary font-bold">{paymentTypes.length}</span> of <span className="text-secondary font-bold">{totalItems}</span> records
          </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-base-100 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col border-t-8 border-primary animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-8">
              <div className="grid grid-cols-1 gap-6">
                ...
                <div className="form-control">
                  <label className="label-text font-bold mb-1">Payment Type Name *</label>
                  <input 
                    required 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="form-control">
                    <label className="label-text font-bold mb-1">Reference Name *</label>
                    <input 
                      required 
                      className="input input-bordered focus:border-primary w-full" 
                  ...
                  <div className="form-control">
                    <label className="label-text font-bold mb-1">Payee Name *</label>
                    <input 
                      required 
                      className="input input-bordered focus:border-primary w-full" 
